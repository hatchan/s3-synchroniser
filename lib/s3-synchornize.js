...
var _uploadFiles = function(sourceS3, destinationS3, sourceBucket, destinationBucket, files, callback) {
    async.eachLimit(files, CONCURRENT_UPLOADS, function(file, next) {
        ...
        destinationS3.putObject(destinationParams, function(err, data) {
            if (err) {
                console.error("Unable to put object " + file.Key + " to bucket " + destinationBucket);
                next(err);
                return void(0);
            }
            
            console.error("Uploaded object '" + file.Key + "' to bucket '" + destinationBucket + "'.");
...
exports.synchronizeS3Buckets = function(sourceBucket, destinationBucket, options, callback) {
    ...
    var uploadMissingItems = function(next, results) {
        ...
        _uploadFiles(sourceS3, destinationS3, sourceBucket, destinationBucket, files, next);
    };
